import { getNowPlaying } from "@/old/lib/services/spotify";
...
export default async function NowPlaying() {
  ...
  return (
    <div className="mb-8 flex w-full flex-row-reverse space-x-0 sm:flex-row sm:space-x-2">
      <svg className="ml-auto mt-1 h-4 w-4" viewBox="0 0 168 168">
        <path
          fill="#1ED760"
      ...
      <div className="inline-flex w-full max-w-full flex-col truncate sm:flex-row">
        ...
        <span className="mx-2 hidden text-[#1ED760]/70 sm:block">{" – "}</span>
        <p className="max-w-max truncate text-[#1ED760]/70">
          {data?.artist ?? "Spotify"}
        </p>
      </div>
    </div>
