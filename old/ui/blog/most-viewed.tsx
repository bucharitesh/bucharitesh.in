...
export default function MostViewed() {
  ...
  return (
    <div className="space-y-6 bg-primary-400/5 p-4 rounded-xl">
      <div>
        <div className="flex items-start gap-2 mb-2.5 font-bold text-xs lowercase text-primary">
          Trending
          <ZapIcon />
        </div>
      </div>
...
const TrendingPostItem = ({ post }: { post: any | undefined }) => {
  return (
    <a
      href={`/blog/${post.slug}`}
      className={cn(
        "block text-primary/50 transition-all hover:text-primary-400 pl-3"
      )}
    >
