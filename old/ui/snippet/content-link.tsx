...
function Title({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-xl text-primary/90 transition duration-300 line-clamp-2 hover:text-primary/90">
      {children}
    </h3>
  );
}
...
function Icon(props: { icon: ElementType }) {
  return (
    <div className="mt-1 ml-2 shrink-0">
      <props.icon className="w-5 text-primary/30 transition-colors hover:text-primary/50" />
    </div>
  );
}
...
function Text({
  ...
  return (
    <p className={cn("mt-4 text-lg text-primary/70 line-clamp-3", className)}>
      {children}
    </p>
  );
}
...
function Meta({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-wrap space-x-2 text-base text-primary/50">
      {children}
    </div>
  );
}
