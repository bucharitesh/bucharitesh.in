...
import { Dock, DockIcon, DockIconActiveDot } from "@/ui/layout/floating-dock";
...
const DATA = {
  navbar: [
    ...
    { href: "/craft", icon: PencilIcon, label: "Craft" },
  ],
  contact: {
    social: {
      GitHub: {
    ...
    <Dock className="fixed bottom-2 left-1/2 -translate-x-1/2">
      {DATA.navbar.map((item) => (
        <DockIcon key={item.label} title={item.label}>
          ...
          {pathname === item.href && <DockIconActiveDot isActive={pathname === item.href} />}
        </DockIcon>
      ))}
      <Seperator />
