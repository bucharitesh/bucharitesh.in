...
const PolyrhythmicSpiral: React.FC = () => {
  ...
  const getUrl = (index: number): string => `https://assets.codepen.io/1468070/${getFileName(index)}.wav`;
  ...
  const calculateNextImpactTime = (currentImpactTime: number, velocity: number): number => {
  ...
  const determineOpacity = (currentTime: number, lastImpactTime: number): number => {
  ...
  const playKey = (index: number): void => {
    try {
      if (keysRef.current && keysRef.current[index]) {
        ...
        audio.play().catch(err => console.error(`Sound play error for ${index}:`, err));
      }
    } catch (error) {
  ...
  const draw = (): void => {
    ...
    pen.clearRect(0, 0, dimensionsRef.current.width, dimensionsRef.current.height);

    const currentTime = new Date().getTime();
    const elapsedTime = (currentTime - settings.startTime) / 1000;

    const length = Math.min(dimensionsRef.current.width, dimensionsRef.current.height) * 0.9;
