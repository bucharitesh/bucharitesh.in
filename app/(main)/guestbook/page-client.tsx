...
const PageClient = ({ session }: any) => {
  const { comments, onDelete } = useComments();
...
function WordsEntry({ comment, user, handleDelete }: any) {
  return (
    <div className="relative flex w-full flex-col space-y-4">
      <div className="prose w-full break-words prose-dark text-primary-500">
        {comment.message}
      </div>
      <div className="flex items-center space-x-3">
        ...
        <p className="text-sm text-primary-400/90">{comment.name}</p>
